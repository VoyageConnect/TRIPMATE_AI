...
class MatchingService:
    ...
    def generate_match_description(self, user1: User, user2: User, score: float, distance: float) -> str:
        try:
            prompt = f"""
            두 사용자의 매칭 결과에 대한 설명을 생성해주세요:

            사용자1의 선호도:
            - 문화관광: {user1.cultural_tourism}/5
            - 음식: {user1.food}/5
            - 역사관광: {user1.historical_tourism}/5
            - 레저스포츠: {user1.leisure_sports}/5
            - 자연관광: {user1.nature_tourism}/5
            - 쇼핑: {user1.shopping}/5

            사용자2의 선호도:
            - 문화관광: {user2.cultural_tourism}/5
            - 음식: {user2.food}/5
            - 역사관광: {user2.historical_tourism}/5
            - 레저스포츠: {user2.leisure_sports}/5
            - 자연관광: {user2.nature_tourism}/5
            - 쇼핑: {user2.shopping}/5

            매칭 점수: {score:.2f}
            거리: {distance:.2f}km

            이 정보를 바탕으로 두 사용자가 잘 맞는 이유와 함께 추천하는 활동을 설명해주세요.
            답변은 100자 이내로 작성해주세요.
            """
    ...
    def handle_matching_request(self, request: Dict) -> Dict:
        try:
            ...
            logger.debug(f"Received request: user_id={user_id}, latitude={latitude}, longitude={longitude}")

            # 먼저 이미 매칭된 결과가 있는지 확인
            ...
            while time.time() - start_time < 10:  # 10초 대기
                ...
                try:
                    current_match, current_score, current_distance = self.find_best_match(user, max_distance=10)
                    ...
                    if current_score > best_score:
                        ...
                        try:
                            ...
                            logger.error(f"Error generating match description: {e}")
                            description = "두 사용자의 여행 성향이 잘 맞아 즐거운 여행이 될 것 같습니다."
                        ...
                        # 상대방을 위한 결과도 동일하게 저장
                        self.matching_results[best_match.user_id] = {
                            "result": "success",
